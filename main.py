...
def generate_image(input_image: Image, tile_size: int, neighborhood: Sequence[tuple[int, int]], width: int = -1, height: int = -1) -> Image:
    ...
    width, height = horizontal_tiles if width < 0 else width, vertical_tiles if height < 0 else height

    superposition = get_superposition(tile_frequencies, width, height)

    def propagation_implementation(c: COORDINATES):
        return propagate(c, superposition, constraints, neighborhood, width, height)

    iterations = 0
    while (coordinates := get_lowest_entropy(superposition)) is not None:
        collapse(coordinates, superposition)
        flood_fill(coordinates, neighborhood, width, height, propagation_implementation)
        iterations += 1
        print(iterations)

    return get_image(superposition, tile_size, width, height)
...
def main():
    ...
    output_image = generate_image(input_image, tile_size, neighborhood, width=-1, height=-1)
